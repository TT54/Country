...
public class CmdAlly extends SubCommand {
    ...
    public CmdAlly() {
        super("ally", new String[]{}, "Allow to send/accept an ally request to other countries");
    }
    ...
    public boolean onExecute(CommandSender sender, SubCommand subCommand, String command, String[] args) {
        ...
        if (country.hasRelation(target.getUuid(), Relations.ALLY)) {
            player.sendMessage(Main.getMessages().getMessage("alreadythisrelation", "%country%", target.getName()));
            return false;
        }

        RelationManager.sendAllyRequest(country, target);
        return true;
    }
