...
public class CmdNeutral extends SubCommand {

    public CmdNeutral() {
        super("neutral", new String[]{}, "Allow to set relations to neutral with another countries");
    }
    ...
    public boolean onExecute(CommandSender sender, SubCommand subCommand, String command, String[] args) {
        ...
        if (country.hasRelation(target.getUuid(), Relations.NEUTRAL)) {
            player.sendMessage(Main.getMessages().getMessage("alreadythisrelation", "%country%", target.getName()));
            return false;
        }

        RelationManager.setNeutral(player, country, target);
        return true;
    }
