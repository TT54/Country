...
public class CmdEntente extends SubCommand {

    public CmdEntente() {
        super("entente", new String[]{}, "Allow to send/accept an entente request to other countries");
    }
    ...
    public boolean onExecute(CommandSender sender, SubCommand subCommand, String command, String[] args) {
        ...
        if (country.hasRelation(target.getUuid(), Relations.ENTENTE)) {
            player.sendMessage(Main.getMessages().getMessage("alreadythisrelation", "%country%", target.getName()));
            return false;
        }

        RelationManager.sendEntenteRequest(country, target);
        return true;
    }
